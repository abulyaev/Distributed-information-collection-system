...
int main(int argc, char * argv[])
{
	...
	std::cout << "Enter IP-address: "; // Просим ввести ip-адрес сервера
	...
	if (connect(ClientSocket, (SOCKADDR *)&ServerAddress, sizeof(ServerAddress)) == SOCKET_ERROR)
	{
		...
		std::cout << ("Error connect %d\n", WSAGetLastError());
		return -1;
	}
	std::cout << "connected to " << addr << std::endl << std::endl;
	...
	// Получение дескриптора криптопровайдера. (в hProv)
	//Get cryptoprovider descriptor
	...
	while (choice != 8)
	{
		std::cout << "\n"
			<< "1. get os\n"
			<< "2. current os time\n"
			<< "3. time since os started\n"
			<< "4. memory\n"
			<< "5. free memory in local disks \n"
			<< "6. Access rights \n"
			<< "7. SID \n"
			<< "8. Exit\n";
		std::cin >> choice;
		...
		case 3:
			...
			time = atoi(answer3);
			std::cout << "Time since you tirn on OS:\n" << std::endl;
